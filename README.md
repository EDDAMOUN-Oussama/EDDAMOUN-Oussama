<h1 align="center">Hi, I'm Oussama Eddamoun 👋</h1>

<p align="center">
  <strong>Software Engineering Student · Full-Stack Developer · 42 Network Learner</strong>
</p>

<p align="center">
  Building reliable software across web development, backend systems, C/C++, Java, and AI/data-driven projects.
</p>

---

## About Me

- 🎓 Master's-level Computer Science / Software Engineering student
- 💻 42 Network learner with a strong focus on C, C++, algorithms, systems, and problem solving
- 🌐 Interested in full-stack development, backend engineering, APIs, databases, and distributed systems
- 🤖 Exploring AI, data-driven applications, and practical automation
- 🐳 Building experience with Docker, containers, and modern development workflows
- 📚 I value clean code, strong fundamentals, documentation, testing, and continuous learning

## Tech Stack

### Languages

![C](https://img.shields.io/badge/C-111827?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-111827?style=flat-square&logo=cplusplus&logoColor=white)
![Java](https://img.shields.io/badge/Java-111827?style=flat-square&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-111827?style=flat-square&logo=javascript&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-111827?style=flat-square&logo=typescript&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-111827?style=flat-square&logo=php&logoColor=white)
![Python](https://img.shields.io/badge/Python-111827?style=flat-square&logo=python&logoColor=white)

### Web & Backend

![Node.js](https://img.shields.io/badge/Node.js-111827?style=flat-square&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-111827?style=flat-square&logo=express&logoColor=white)
![Java EE](https://img.shields.io/badge/Java%20EE-111827?style=flat-square&logo=openjdk&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-111827?style=flat-square&logo=dotnet&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-111827?style=flat-square&logo=mysql&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-111827?style=flat-square&logo=sqlite&logoColor=white)

### Tools & Engineering

![Git](https://img.shields.io/badge/Git-111827?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-111827?style=flat-square&logo=github&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-111827?style=flat-square&logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-111827?style=flat-square&logo=linux&logoColor=white)

---

## Featured Work

### [42 C++ Modules](https://github.com/EDDAMOUN-Oussama/cpp-models)
A documented collection of the 42 Network C++ Modules 00–09, covering OOP, memory management, inheritance, polymorphism, exceptions, templates, STL containers, parsing, and algorithms.

### [42 Exam Rank 05](https://github.com/EDDAMOUN-Oussama/exam05)
Exam preparation repository covering `bigint`, `vect2`, `polyset`, Conway's Game of Life, and BSQ, with exercise-specific documentation and testing tools.

### Java Projects
A growing collection of Java exercises and systems work. The public repository is currently being reorganized and documented for clearer presentation.

---

## Current Focus

```text
Full-Stack Engineering
Backend & API Design
C / C++ Problem Solving
Java & Enterprise Development
Docker & DevOps Fundamentals
AI / Data-Driven Applications
```

---

## Engineering Principles

```text
Understand the problem
        ↓
Design before coding
        ↓
Keep the implementation readable
        ↓
Test normal and edge cases
        ↓
Document the important decisions
        ↓
Improve through iteration
```

---

## Connect

- GitHub: [@EDDAMOUN-Oussama](https://github.com/EDDAMOUN-Oussama)
- LinkedIn: Add your LinkedIn URL here
- Portfolio: Add your portfolio URL here

---

<p align="center">
  <em>Always learning, building, and improving.</em>
</p>
